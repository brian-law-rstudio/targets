...
For example, let's Knit this entire document with this code chunk commented out and see if the `child_report.html` is actually generated in the File pane.

```{r}
xfun::Rscript_call(
  rmarkdown::render,
  list(input = './child_report.Rmd')
)
```

Now let's Knit this document with the render line above NOT commented out and see if the `child_report.html` is actually generated in the File pane. Nope.
